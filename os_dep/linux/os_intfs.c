...
/*
static int rtw_proc_show(struct seq_file *m, void *v)
{
        seq_printf(m, "0x%08X\n", rtw_proc_cnt);
        return 0;
}
...
{
        return single_open(file, rtw_proc_show, NULL);
}

static ssize_t rtw_proc_write(struct file *file,
                const char __user *buffer, size_t count, loff_t *pos)
{
        return 0;
}

static ssize_t debug_get_drv_version(struct file *file, char __user *buf,
                        size_t nbytes, loff_t *pos)
{
        return 0;
}

static const struct file_operations rtwinfo_proc_fops = {
        .owner          = THIS_MODULE,
        .open           = rtw_proc_open,
        .read           = proc_get_drv_version,
        .llseek         = seq_lseek,
...
/*///-----------------todo from here-----------------
	...
	if(rtw_proc == NULL)
	{
		...
		entry = proc_create("ver_info", S_IFREG | S_IRUGO, rtw_proc, &rtwinfo_proc_fops);
...
u32 rtw_start_drv_threads(_adapter *padapter)
{
	...
	padapter->xmitThread = kthread_run(rtw_xmit_thread, (void *)padapter, "rtx_xmit", CLONE_FS|CLONE_FILES);
	...
	padapter->recvThread = kthread_run(rtw_recv_thread, (void *)padapter, "rtw_recv", CLONE_FS|CLONE_FILES);
	if(padapter->recvThread < 0)
		_status = _FAIL;	
#endif

	//padapter->cmdThread = kernel_thread(rtw_cmd_thread, padapter, CLONE_FS|CLONE_FILES);
	padapter->cmdThread = kthread_run(rtw_cmd_thread, (void *)padapter,"rtw_cmd", CLONE_FS|CLONE_FILES);
	...
	padapter->evtThread = kthread_run(event_thread, (void *)padapter, "event_thread", CLONE_FS|CLONE_FILES);
